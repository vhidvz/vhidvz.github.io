...
In this library, I have scoped `action` and `subject` which means you can have more control over these attributes.
...
In microservices design patterns and restful's one of the best practices has focused on resource management, a single endpoint with a concentration on subjects is better than having multiple endpoints.
